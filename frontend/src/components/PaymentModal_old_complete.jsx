/**
 * PaymentModal.jsx - Layer 2 Gas-Free TeoCoin Integration
 * VERSION: 4.0 - Layer 2 gas-free implementation
 * LAST UPDATED: 2025-07-08 (Layer 2 Integration)
 * 
 * This component provides truly gas-free TeoCoin discounts using Layer 2 infrastructure
 */
...
const DISCOUNT_CONTRACT_ADDRESS = '0xd30afec0bc6ac33e14a0114ec7403bbd746e88de'; // TeoCoin Discount Contract
...
const PaymentForm = ({ course, onSuccess, onClose, onError }) => {
    ...
    useEffect(() => {
        ...
        // CLEAR PAYMENT SUMMARY CACHE to force fresh API call
        const cacheKey = `payment_summary_${course.id}`;
        paymentSummaryCache.delete(cacheKey);
        
        console.log('🧹 Cleared stale discount data and payment cache on modal open');
    }, [course.id]);
    ...
    const fetchPaymentSummary = useCallback(async () => {
        try {
            ...
            if (response.data.success) {
                const summaryData = response.data; // The data is directly in response.data, not response.data.data
            ...
            onError(`Failed to load payment options: ${error.message}`);
        } finally {
            setLoading(false);
        }
    ...
    const updateTeoInfo = async (provider, address) => {
        try {
            ...
            // Determine which contract needs allowance based on payment type
            const teoOption = (paymentSummary?.pricing_options || []).find(opt => opt.method === 'teocoin');
            const isTeoCoinDiscount = teoOption && teoOption.discount;
            const spenderAddress = isTeoCoinDiscount ? DISCOUNT_CONTRACT_ADDRESS : REWARD_POOL_ADDRESS;
            
            // Get allowance for the appropriate contract
            const allowanceWei = await contract.allowance(address, spenderAddress);
    ...
    const approveTeoCoin = async (amount) => {
        try {
            ...
            console.log('🔑 Requesting approval for', amount, 'TEO to correct contract');
            
            // Determine which contract needs approval based on payment type
            const teoOption = (paymentSummary?.pricing_options || []).find(opt => opt.method === 'teocoin');
            const isTeoCoinDiscount = teoOption && teoOption.discount;
            const spenderAddress = isTeoCoinDiscount ? DISCOUNT_CONTRACT_ADDRESS : REWARD_POOL_ADDRESS;
            const contractName = isTeoCoinDiscount ? 'Discount Contract' : 'Reward Pool';
            
            console.log(`   Spender: ${contractName} (${spenderAddress})`);
            console.log(`   TeoCoin Discount Mode: ${isTeoCoinDiscount ? 'Yes' : 'No'}`);
            ...
            try {
                ...
                tx = await contract.approve(spenderAddress, amountWei, {
                    gasLimit: gasLimit
                });
                ...
                try {
                    ...
                    const txRequest = {
                        ...
                        data: contract.interface.encodeFunctionData('approve', [spenderAddress, amountWei]),
                        gasLimit: gasLimit,
                    ...
                    try {
                        const populatedTx = await contract.approve.populateTransaction(spenderAddress, amountWei);
    ...
    // Enhanced TeoCoin payment flow with correct business logic
    const handleTeoCoinPayment = async () => {
        try {
            ...
            const discountPercent = teoOption.discount_percent || 10; // Default 10% if not specified
            
            // Get the original course price from the fiat option
            const fiatOption = (paymentSummary?.pricing_options || []).find(opt => opt.method === 'fiat');
            const originalPrice = fiatOption ? parseFloat(fiatOption.price) : 99.99; // Fallback price
            
            const discountAmount = originalPrice * discountPercent / 100;
            const finalPrice = originalPrice - discountAmount;
            
            console.log('💰 TeoCoin Payment Flow Started (Correct Business Logic)');
            console.log('💰 Original price:', originalPrice);
            console.log('💸 Discount amount:', discountAmount);
            console.log('💳 Student pays immediately:', finalPrice);
            console.log('🔔 Creating teacher notification for EUR vs TEO choice');

            // Step 1: Create discount request (generates teacher notification)
            console.log('� Creating discount request for teacher...');
            const discountRequest = await createDiscountRequest(discountAmount, discountPercent, originalPrice);
            console.log('✅ Discount request created:', discountRequest.requestId);

            // Step 2: Create payment intent for the DISCOUNTED amount (student pays immediately)
            const { createPaymentIntent } = await import('../services/api/courses');
            const response = await createPaymentIntent(course.id, {
                teocoin_discount_request_id: discountRequest.requestId,
                discount_amount: discountAmount,
                final_amount: finalPrice,
                payment_method: 'fiat_with_teocoin_discount',
                wallet_address: walletAddress,
                teacher_wallet: course.teacher.wallet_address
            });

            console.log('📝 Payment intent response:', response);

            if (response.data.success) {
                // Store discount info for payment completion
                const discountInfo = {
                    discount_request_id: discountRequest.requestId,
                    discount_amount: discountAmount,
                    discount_percent: discountPercent,
                    final_amount: finalPrice,
                    client_secret: response.data.client_secret,
                    student_gets_discount: true, // Student ALWAYS gets discount
                    teacher_chooses_eur_vs_teo: true // Teacher will choose EUR vs TEO
                };
                ...
                // Show success message explaining the correct business logic
                alert(`✅ TeoCoin discount applied successfully!
                
💰 Original price: €${originalPrice.toFixed(2)}
💸 Your discount: €${discountAmount.toFixed(2)} (${discountPercent}%)
💳 You pay now: €${finalPrice.toFixed(2)}

🎓 You will be enrolled immediately!
🔔 Your teacher will be notified to choose:
   💶 Keep €${discountAmount.toFixed(2)} (safe option)
   🪙 Accept TEO staking (earn bonus)

Complete your discounted payment now.`);
            } else {
                throw new Error(response.data.error || 'TeoCoin discount payment failed');
            }
    ...
    const handleApproval = async (amount) => {
        try {
            ...
            console.log('🔑 Requesting approval for', amount, 'TEO to', contractName, `(${spenderAddress})`);
            console.log(`   TeoCoin Discount Mode: ${isTeoCoinDiscount ? 'Yes' : 'No'}`);
            ...
            const userConfirmed = confirm(
                `Approve ${amount} TEO spending for ${contractName}?\n\n` +
                `This will allow the ${contractName} to spend ${amount} TEO from your wallet.\n` +
    ...
    // Create discount request via backend API (no MetaMask transactions needed)
    const createDiscountRequest = async (amount, discountPercent, originalPrice) => {
        try {
            // Import the TeoCoin discount API service
            const { createDiscountRequest: createRequest, generateSignatureData } = await import('../services/api/teocoinDiscount');
            
            console.log('💰 Creating discount request for course:', course.id);
            console.log('📋 Student:', walletAddress, 'Teacher:', course.teacher?.wallet_address);
            console.log('🔍 Course object:', course);
            console.log('👨‍🏫 Teacher object:', course.teacher);
            
            // Generate signature data for student approval
            const signatureData = await generateSignatureData(
                walletAddress,
                course.id,
                originalPrice,  // Use the correct price from payment summary
                discountPercent
            );
            
            // Show user confirmation for signature
            const userConfirmed = confirm(
                `Sign TeoCoin discount request?\n\n` +
                `Course: ${course.title}\n` +
                `Discount: ${discountPercent}% (€${(originalPrice * discountPercent / 100).toFixed(2)})\n` +
                `TEO Cost: ${signatureData.teo_cost / Math.pow(10, 18)} TEO\n\n` +
                `You will pay the discounted price now. Your teacher will then choose between EUR or TEO.`
            );
            ...
            // Get user signature (this doesn't cost gas - just signs a message)
            const signer = await web3Provider.getSigner();
            
            // CRITICAL FIX: Approve TEO spending BEFORE creating discount request
            console.log('🔐 Approving TEO spending for discount contract...');
            const teoAmount = signatureData.teo_cost / Math.pow(10, 18);
            console.log(`💰 Approving ${teoAmount} TEO for spending`);
            
            try {
                // Approve TEO spending with extra buffer to account for any calculation differences
                const approvalAmount = teoAmount * 1.1; // 10% buffer for safety
                const approvalTxHash = await approveTeoCoin(approvalAmount);
                console.log('✅ TEO spending approved, transaction hash:', approvalTxHash);
                
                // CRITICAL: Wait for approval transaction to be confirmed before proceeding
                console.log('⏳ Waiting for approval transaction to be confirmed...');
                const provider = web3Provider;
                let receipt = null;
                let attempts = 0;
                const maxAttempts = 60; // 60 seconds timeout (increased for safety)
                
                while (!receipt && attempts < maxAttempts) {
                    try {
                        receipt = await provider.getTransactionReceipt(approvalTxHash);
                        if (receipt) {
                            if (receipt.status === 1) {
                                console.log('✅ Approval confirmed in block:', receipt.blockNumber);
                                console.log('💰 Gas used:', receipt.gasUsed.toString());
                                ...
                                throw new Error('Approval transaction failed (reverted)');
                            }
                        }
                    } catch (receiptError) {
                    ...
                    if (attempts % 5 === 0) {
                        console.log(`⏳ Still waiting for approval confirmation... (${attempts}/${maxAttempts} seconds)`);
                    }
                }
                
                if (!receipt) {
                    throw new Error('Approval transaction confirmation timeout after 60 seconds. The transaction may still be pending. Please check your wallet and try again in a few minutes.');
                }
                
                // Double-check that allowance is now sufficient
                await updateTeoInfo(web3Provider, walletAddress);
                console.log('✅ Approval transaction confirmed, allowance updated');
                
                // ADDITIONAL SAFETY: Wait a bit more for blockchain state to propagate
                console.log('⏳ Waiting additional 3 seconds for blockchain state propagation...');
                await new Promise(resolve => setTimeout(resolve, 3000));
                
                // Final allowance verification
                console.log('🔍 Final allowance verification before backend call...');
                await updateTeoInfo(web3Provider, walletAddress);
                
            } catch (approvalError) {
                console.error('❌ TEO approval failed:', approvalError);
                throw new Error(`Failed to approve TEO spending: ${approvalError.message}`);
            }
            
            // Use the signable message from the backend (this must match what the smart contract expects)
            const messageToSign = signatureData.signature_data.signable_message;
            
            console.log('🔍 Debug - Message to sign:', messageToSign);
            console.log('🔍 Debug - Wallet address:', walletAddress);
            
            // CORRECTED: The smart contract expects a signature over the inner hash WITH Ethereum prefix
            // So we need to sign the inner hash using signMessage which adds the prefix
            // The backend gives us the inner hash, we sign it with prefix
            const signature = await signer.signMessage(ethers.getBytes(messageToSign));
            
            console.log('✅ Student signed with signMessage (correct format):', signature.slice(0, 20) + '...');
            
            // Create the discount request via backend
            const teacherAddress = course.teacher?.wallet_address || course.teacher_wallet_address || course.teacher?.wallet || '';
            
            console.log('🎯 Final teacher address being sent:', teacherAddress);
            
            if (!teacherAddress) {
                throw new Error('Teacher wallet address not found. The course teacher needs to connect their wallet to enable TeoCoin discounts.');
            }
            ...
            const discountRequest = await createRequest({
                ...
                courseId: course.id,
                coursePrice: originalPrice,  // Use the correct price
                discountPercent: discountPercent,
                studentSignature: signature
            });
            
            console.log('✅ Discount request created:', discountRequest.request_id);
            
            return {
                requestId: discountRequest.request_id,
                transactionHash: discountRequest.transaction_hash,
                discountAmount: originalPrice * discountPercent / 100  // Use the correct price
            };
            ...
            if (error.message.includes('user rejected') || error.message.includes('cancelled')) {
                throw new Error('Discount request was cancelled by user');
            } else if (error.message.includes('Insufficient TEO balance')) {
                throw new Error('Insufficient TEO balance for discount request');
            } else {
                throw new Error(`Discount request failed: ${error.message}`);
            }
        }
    };
            ...
            <div className="payment-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="payment-actions">
                    ...
                    >
                        {processing ? '⏳ Processing...' : 
                         ...
                         paymentMethod === 'teocoin' && discountApplied ? '✅ Discount Ready' :
                         paymentMethod === 'teocoin' && approvalStatus === 'pending' ? '⏳ Signing Request...' :
                         paymentMethod === 'teocoin' ? '🪙 Get TeoCoin Discount' :
                ...
                {paymentMethod === 'teocoin' && (
                    <div className="teocoin-info">
                        ℹ️ You'll pay the discounted price immediately. Your teacher will then choose EUR vs TEO staking.
                    </div>
                )}

                {discountApplied && paymentMethod === 'fiat' && (
                    <div className="discount-applied-info">
                        ✅ TeoCoin discount ready! Complete your purchase at the reduced price.
                    </div>
                )}
            </div>
        </div>

...
class Web3Service {
  ...
  async getMaticBalance(address = null) {
    ...
    try {
      ...
      if (this.isWalletLocked && !address) {
        console.log('🔒 MATIC balance ottenuto per wallet bloccato:', targetAddress, 'Balance:', ethers.formatEther(balance));
      }
      
      return ethers.formatEther(balance);
      
    } catch (error) {
      console.error('Errore nel recupero balance MATIC per indirizzo', targetAddress, ':', error);
   ...
   */
  async checkMaticForGas(minMaticRequired = '0.01') {
    try {

...
def trigger_achievement_reward(request):
    ...
    try:
        ...
        # Validate achievement type (basic validation)
        valid_achievement_types = [
            'first_course_completed', 'perfect_scores', 'fast_completion', 
            'helpful_peer', 'consecutive_logins'
        ]
        if achievement_type not in valid_achievement_types:
...
def bulk_process_rewards(request):
    ...
    try:
        ...
        if reward_type not in valid_achievement_types:
        ...
        for user_id in user_ids:
            try:
                ...
                # Process the reward based on type (no course required for bulk operations)
